...
Focus Hold review and result presentation translate Runtime facts into practitioner decisions. Review asks whether the same shape was kept and shows only the wander count; result shows hold duration, wander count, and same/changed target. Raw event counts, evidence-fact counts, expected-fact ratios, and transient completion-loading panels are not practitioner-facing information. The review stays visible until the processed result is ready.
