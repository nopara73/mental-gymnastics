...
## Pre-UI App Integration Reuse

Pre-UI app workflows live in `src/MentalGymnastics.App` and are documented in [docs/app-integration-boundary.md](docs/app-integration-boundary.md). Before adding Android, CLI, or host behavior that touches startup configuration, first-run initialization, current training state, allowed next work, generated content preparation, runtime session preparation, completed-session processing, active runtime session snapshots, or progress refresh, use the app integration layer instead of wiring Core, Persistence, Runtime, and Content directly in a frontend.

Android screens should consume `MentalGymnastics.App` services and read models, render their output, and forward user actions into app/runtime commands. They must not create parallel orchestration paths, duplicate local JSON stores, replace the current JSON persistence with SQLite or Room without an explicit storage requirement, maintain independent timers or cue schedules, generate prompts in screens, or grant progress from UI-local state. If a workflow is missing, add it in `MentalGymnastics.App` with tests and keep lower-layer responsibilities in their owning projects.

## Android UI Layer

The Android UI layer is documented in [docs/android-ui-layer.md](docs/android-ui-layer.md). Before adding or changing Android activities, navigation, views, components, lifecycle handling, session screens, progress surfaces, evidence screens, maintenance/decay screens, global review screens, or backup/restore UI, read that document and preserve the Android ownership boundary.

Android owns native presentation, navigation, accessibility, platform lifecycle forwarding, and ephemeral UI state such as selected tabs, expanded detail, response text, loading/error display, and local confirmation state. Android must not reimplement app workflows, Core domain rules, Persistence database logic, Runtime timers/cue schedules/session protocols/evidence logs, or Content prompt generation/freshness/equivalence logic inside `Activity`, shell, view, or component code.

Use `MentalGymnastics.App` workflows and Android-facing app snapshots for startup, current training state, next work, session preparation, live session commands, completion processing, active session snapshot restore/invalidation, progress refresh, and local backup/restore. If a screen needs data or behavior not exposed by the app layer, add the workflow or read model to `MentalGymnastics.App` with tests, or add the missing rule/storage/runtime/content behavior to the owning lower layer with tests first.
